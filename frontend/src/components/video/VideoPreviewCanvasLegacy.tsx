...
export function VideoPreviewCanvas() {
  ...
  const renderLayer = (
    ...
    // Crop editing renders the full frame with dimmed margins; canonical media
    // geometry, transition paint, and rotation stay on the interaction fallback
    // so handles are not displaced, clipped, faded, or scaled while editing.
    const inCropEdit = cropMode && selected && isMedia && !isPlaying;
    const transitionPaint = resolvePreviewFrameOwnerTransitionPaint(
      entry.canonicalState,
      hasLiveOverride || inCropEdit,
    );
