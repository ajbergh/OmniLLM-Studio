...
|---|---|---:|---|---|---|
| 0 | Architecture, threat model, durable roadmap | P0 | **COMPLETE** | PR #98 — `agent/sandbox-foundation-roadmap-20260812` | Architecture + threat model + tracker implemented. PR #98 Quality Gate and Security Scan passed before final tracker refresh. |
...
| 2 | First-party runtime abstraction + Linux execution plane | P1 | **IN PROGRESS** | `agent/sandboxd-linux-runtime-20260812` | Linux Bubblewrap runtime and authenticated `sandboxd` worker implemented with focused tests; PR/CI and integration fixture remain. |
| 3 | Immediate subprocess hardening for stdio MCP/plugins | P0 | **IN PROGRESS** | PR #99 — `agent/sandbox-subprocess-boundary-20260812` | `os.Environ()` inheritance removed, common sanitized runner seam added, ambient-secret regression tests added. Implementation head passed Quality/Security/Container; tracker refresh/merge remains. |
| 4 | Route `code_execute` + `python_analysis` through Broker | P1 | NOT STARTED | TBD | Existing public tool contracts remain stable while execution becomes ownership-bound and OS-sandboxed. |
...
4. Linux first-party runtime → `agent/sandboxd-linux-runtime-20260812`, stack on #101 then retarget after #101 merges
