...
- Layer-based timeline: clip move (group-aware), trim, split (playhead, blade tool, or split-all), constant **0.25×–4× clip speed** and target-duration retiming for video/audio (source span is preserved; ripple mode moves following clips), duplicate, delete, copy/cut/paste (Ctrl+C/X/V, paste-at-playhead or paste-here), copy/paste clip attributes, multi-select (modifiers, Ctrl+A, before/after playhead, all-on-layer), grouping (`G`/`Shift+G`), align/distribute, markers (toolbar button or `M`), snapping with drop guides colored and labeled by snap target (playhead / marker / clip edge / scene boundary / timeline edge), undo/redo, zoom (buttons, `+`/`-`/`F`, Ctrl+wheel), layer add/remove/rename/reorder/duplicate/clear/solo (persisted for preview and export audio), sticky headers, drag-to-resize row height, and a `?` shortcut overlay.
...
- Preview canvas composites **all visible tracks** at the playhead (track order + z-index) with transforms, opacity, fades, crop, CSS-filter effects, keyframe animation, and styled text. Visual video elements are muted and every active audio/video asset is auditioned through the managed preview-audio path, so clip volume, volume keyframes, fades, solo, speed, and the footer's preview-only master volume apply consistently. Direct manipulation: an **8-handle selection box** (uniform scale for media/text anchored at the opposite corner/edge, true width/height resize for shapes) plus a rotate handle, with Shift (constrain aspect), Alt (resize from center), and Ctrl (bypass snapping) modifiers and a live dimension/position readout; **smart guides** snap moves to canvas center/edges/safe-area bounds and other objects' edges/centers; **double-click text clips to edit inline** (Enter commits, Shift+Enter newline, Escape cancels); **8-handle crop mode** with a thirds grid, dimmed margins, and explicit Apply/Cancel/Reset controls; safe-area/grid overlays and a fullscreen toggle.
- Inspector with timecode-capable timing fields (start/end/duration/trim-in), **speed percent and target-duration retiming** for video/audio, 2D/2.5D transforms, crop, text styling, searchable effect/transition browsers with runtime export-support badges, and editable easing/Bezier/spring keyframes. The fifth **Motion Design** mode organizes the right rail as **Design / Animate / Effects / AI / Export** and provides a single 21-block In/During/Out animation registry whose generated keyframes remain editable; other modes retain their existing gated tabs.
...
Video Edit Studio exports through persisted backend render jobs. The default renderer uses FFmpeg to composite real video and image media, text/caption/callout clips, and canvas settings into durable MP4/WebM export assets — including constant 0.25×–4× video/audio retiming (with pitch-preserving audio), 2.5D position/scale/rotation/camera projection, opacity, fades, fade-style/slide/wipe/zoom transitions, cinematic scene effects, deterministic easing/Bezier/spring keyframes, blur/pixelate redaction regions, persisted track solo, audio mixing, and text styling. Export settings additionally support an H.265 codec option, audio bitrate, caption burn-in toggle, SRT/VTT sidecar caption assets, and timeline-range exports. Remaining partial features—such as true X/Y tilt, true crossfades, complex vector annotations, click-audio synthesis, drop shadow, and background blur—are reported by `GET /v1/video/render/capabilities`, which drives inspector fidelity warnings and the pre-render checklist.
...
| `POST /v1/video/projects/{projectId}/assistant/edit-plan` | Generates a timeline-ID/revision-bound edit plan from a natural-language instruction. Calls the LLM to produce typed `EditOperation` objects; falls back to keyword matching. |
| `POST /v1/video/projects/{projectId}/assistant/apply-plan` | Atomically applies a validated, current-revision `EditPlan`; stale or invalid plans make no change. |
