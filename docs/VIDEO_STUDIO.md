...
- Layer-based timeline: clip move (group-aware), trim, split (playhead, blade tool, or split-all), duplicate, delete, copy/cut/paste (Ctrl+C/X/V, paste-at-playhead or paste-here), copy/paste clip attributes, multi-select (modifiers, Ctrl+A, before/after playhead, all-on-layer), grouping (`G`/`Shift+G`), align/distribute, markers (toolbar button or `M`), snapping with drop guides, undo/redo, zoom (buttons, `+`/`-`/`F`, Ctrl+wheel), layer add/remove/rename/reorder/duplicate/clear/solo (preview audio), sticky headers, drag-to-resize row height, and a `?` shortcut overlay.
...
- **Annotation library** — 14 shape kinds (rectangle, highlight, blur, pixelate, rounded rectangle, ellipse, arrow, line, speech bubble, spotlight, checkmark, X mark, numbered step marker, label callout) defined in `frontend/src/components/video/effects/annotationRegistry.ts` with per-kind creation defaults, kind-aware inspector controls (fill/stroke/corner radius/blur-or-block size), export-support badges, and six style presets (yellow highlight, red outline, blue tutorial callout, dark lower-third, keyboard shortcut, numbered step). Blur and pixelate regions redact whatever composites beneath them in both preview and export; rounded rectangles and labels export with square corners; the remaining kinds are preview-only and reported as such.
