...
- **2026-08-17:** Pushed evidence-integrity commit `9308d09`. Quality Gate `32074904557` completed the hosted parity job successfully and uploaded artifact `9303432653` (50,909,617 bytes; `sha256:f8d8b747…f9d0e`). Independent artifact inspection confirms 103/103 preview PNGs and 103/103 rendered PNGs are exactly 640×360, all 103 JSON frame records report matching dimensions, and the report contains 206/206 unique PNG filenames across 103 unique frame-index prefixes. Audio still passes with equal 3,840,000-byte PCM streams, zero offset, 0.999993 correlation, -21.1/-21.1 LUFS, and -3.1/-3.0 dBFS true peak; delivery still passes at 600/600 CFR frames, 30 fps, PTS 0, and 20.0 seconds. The dimension-valid visual baseline intentionally remains 0/103 passing, with 0.182662 mean pixel pass rate, 0 median, and 0.091190 mean SSIM, so these values measure current divergence rather than approved tolerances.
- **2026-08-17:** The same Quality Gate run remains red only because `tests/video-editor-advanced.smoke.spec.ts` times out on all three attempts while waiting for a `tab` named `Captions` after closing advanced tools; the other 29 browser specifications pass. Updated the test to enter the inspector through the current desktop `Captions` tool-rail button and assert the panel heading before checking generated text. Focused Playwright discovery and the frontend production build pass. Local browser execution is blocked before the test by a Windows Playwright web-server race that reports port 4173 occupied and then leaves it free; hosted end-to-end confirmation is pending.
...
| CI artifact job | Implemented and hosted-verified | Quality Gate `32074904557` produced artifact `9303432653` with 103/103 exact 640×360 preview and render captures, 206/206 unique report PNGs, passing audio/delivery gates, and complete toolchain metadata (Go 1.25.13, Node 24.19.0, npm 11.17.0, FFmpeg/FFprobe 6.1.1-3ubuntu5, Playwright 1.62.1, and DejaVu Sans fallback) |
...
1. Push the captions tool-rail smoke repair and this hosted evidence update, then rerun Quality Gate so PR #187 is fully green.
