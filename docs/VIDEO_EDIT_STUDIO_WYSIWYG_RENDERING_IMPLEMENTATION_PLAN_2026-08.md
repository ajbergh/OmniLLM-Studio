...
**Current status:** Phase 1 is complete. Phase 0 now has automated project seeding, a complete 103-frame known-mismatch baseline, independent browser-preview and snapshot PCM with gated EBU R128, decoded-delivery frame/PTS validation, and a CI artifact job. Audio and delivery gates pass on the current live fixture. Draft PR #187 is open; its first hosted run exposed an unrelated Linux router-shutdown cleanup race, and the approved service-lifecycle fix is implemented locally pending hosted verification. Unsupported-audio-boundary coverage, visual review, and final threshold approval remain.

| Phase | Status | Progress note |
|---|---|---|
| Phase 0 — Parity baseline | In progress | Fixture generation, real-project seeding, 103 named visual captures, passing independent PCM timing/correlation/EBU R128 gates, decoded-delivery frame/PTS checks, mismatch reports, and CI artifact wiring are implemented; PR #187 hosted verification is in progress after fixing a backend shutdown prerequisite failure; unsupported-audio-boundary coverage, visual review, and threshold approval remain |
...
| CI artifact job | Implemented; hosted rerun pending | `video-parity-baseline` generates, seeds, captures preview/snapshot frames and PCM, performs a separate delivery render, reports in known-mismatch mode, records toolchain/font metadata, and uploads the complete evidence bundle for 14 days. The first hosted workflow reached all unrelated prerequisites but skipped this job after a backend router-cleanup race; the lifecycle fix is implemented and awaiting rerun |
...
1. Push the approved router/video lifecycle fix to PR #187, rerun the hosted Quality Gate, and inspect the `video-parity-baseline` delivery render and schema-v2 EBU R128 artifacts; confirm pinned Chromium/FFmpeg/font metadata and resolve any remaining Linux-only determinism, timeout, or packaging failures.
