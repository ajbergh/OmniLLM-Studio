...
**Current status:** Phase 1 is complete. Phase 0 now has automated project seeding, a complete 103-frame known-mismatch baseline, independent browser-preview and snapshot PCM with gated EBU R128, decoded-delivery frame/PTS validation, and a CI artifact job. Audio and delivery gates pass on the current live fixture. Draft PR #187 is open; hosted backend tests and the Linux race detector verify the service-lifecycle fix. The released parity job exposed a missing system FFmpeg/FFprobe dependency on Ubuntu 24.04, and the approved workflow dependency fix is implemented locally pending rerun. Unsupported-audio-boundary coverage, visual review, and final threshold approval remain.

| Phase | Status | Progress note |
|---|---|---|
| Phase 0 — Parity baseline | In progress | Fixture generation, real-project seeding, 103 named visual captures, passing independent PCM timing/correlation/EBU R128 gates, decoded-delivery frame/PTS checks, mismatch reports, and CI artifact wiring are implemented; hosted backend/race verification passes and the parity workflow is awaiting rerun with explicit FFmpeg/FFprobe installation; unsupported-audio-boundary coverage, visual review, and threshold approval remain |
...
| Audio metrics | Implemented; provisional gate passing | Report schema v2 gates exact sample count, offset, correlation, sample peak, EBU R128 integrated loudness, and true peak; approximate loudness remains diagnostic only. The live swept-chirp fixture has equal 1,920,000-frame streams, zero offset, 0.999973 correlation, 0.000793 sample-peak difference, -21.1/-21.1 LUFS, and -3.1/-3.0 dBFS true peak. Final tolerance approval and unsupported-audio-boundary coverage remain |
...
| CI artifact job | Implemented; hosted rerun pending | `video-parity-baseline` generates, seeds, captures preview/snapshot frames and PCM, performs a separate delivery render, reports in known-mismatch mode, records toolchain/font metadata, and uploads the complete evidence bundle for 14 days. Hosted backend/race prerequisites now pass; the first released parity attempt exposed missing system FFmpeg/FFprobe, so the job now installs and verifies both before fixture generation |
...
1. Push the explicit FFmpeg/FFprobe CI dependency fix to PR #187, rerun the hosted Quality Gate, and inspect the complete `video-parity-baseline` delivery render and schema-v2 EBU R128 artifacts; confirm Chromium/FFmpeg/font metadata and resolve any remaining Linux-only determinism, timeout, or packaging failures.
