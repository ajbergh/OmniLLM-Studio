...
`parity-playback-canonical-v7` is additive to immutable v6 and contains 21 retained cases: every v6 media/text/weighted/cursor case, one canonical rounded-rectangle playback case, and one unsupported ellipse control. The first hosted v7 run `34101120638` proved the rounded rectangle itself canonical and produced the exact intended ellipse fallback reason `shape-playback-deferred:playback-shape-ellipse:shape-kind-unsupported`, but the run correctly failed the unchanged audio-clock gate because that control was originally placed at 26.8 seconds while `cmd/video-parity-fixture` physically generates `asset-audio.wav` for 24 seconds. No product or acceptance assertion was relaxed.

Pre-tracker repair head `29da506a863fb85883d787e3b1102883f17350a1` moves the v7-only ellipse control to the 1.0–1.59 second audio-active interval between retained v6 observation windows and adds a fixture test that keeps the observation inside both the shape clip and the generated-media duration. The exact tracker-bearing head must now pass the full triggered Quality/Security/browser/platform matrix plus immutable v6 and new v7 playback evidence before #314 can merge.
...
- Focused Vitest coverage locks export-aligned structural admission plus atomic mixed-frame fallback. The new v7 backend fixture validation preserves every v6 case and asserts the ellipse observation remains within the generated 24-second media budget.
- Hosted v7 run `34101120638` proved the canonical rounded-rectangle consumer and exact unsupported-ellipse reason, then failed only because the original 26.8-second ellipse window exhausted the generated 24-second WAV. The fix relocates that control without relaxing the unchanged timeline/audio/consumer assertions.
- Merge remains gated on the exact tracker-bearing #314 head passing Quality/Security/platform assurance, immutable `parity-playback-canonical-v6`, and new `parity-playback-canonical-v7` plus its dedicated shape-consumer/geometry gate.
...
|---|---|---|
| Phase 0 — Reproducible parity baseline | **In progress** | Deterministic 103-frame visual/audio/delivery evidence exists. Focused retained controls include opaque PNG exactness, H.264 decoded-frame/±3 color, partial-alpha PNG ±1, transparent VP9 presentation/±4, project-background ±1, normal-playback media/weighted/text/cursor ownership, immutable FFmpeg project-font selection, resource-backed Chromium↔FFmpeg glyph acceptance, cursor export/smoothing evidence, and rounded-rectangle export evidence. #314 adds v7 rounded-rectangle playback evidence while preserving v6. The 103-frame torture fixture still lacks a project-font resource and second-platform retained font/glyph evidence. |
...
| Phase 3 — Shared preview composition | **In progress** | Canonical deterministic media, transforms/view/perspective, geometry, effects, transitions, resource text, pixelate raster classes, free-running media/weighted/text/cursor playback, and supported mixed composition are retained. #311 proves the first exact rounded-rectangle export subset; #314 is admitting that same subset into normal playback. Remaining shape kinds, further independently evidenced raster classes, and AudioGraph consumption remain before Phase 3 can close. |
...
- `shape-state-v1` owns shape geometry/style. #285–#299 establish the retained pixelate raster classes; #311 adds the first export-proven static-2D rounded-rectangle raster subset and #314 is the corresponding fail-closed normal-playback admission slice. Other shape kinds remain compatibility debt until independently export-proven.
...
1. **Merge #314 only after its final tracker-bearing exact head passes both retained `parity-playback-canonical-v6` and new `parity-playback-canonical-v7`/shape-consumer evidence, plus every Quality/Security/browser/renderer/platform workflow triggered by the PR diff.** The earlier v7 failure was a fixture-media lifetime mismatch and does not justify weakening the audio-clock gate.
