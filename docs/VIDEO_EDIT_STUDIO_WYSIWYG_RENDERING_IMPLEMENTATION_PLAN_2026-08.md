...
Latest merged WYSIWYG program PR: **#311 — Preserve rounded rectangle geometry in video export** — squash merge `f365da67448a0c3b58eba2727dc6faf3da89ca40`. Exact final head `a4ff85ef0ab5caa47d0f21dda22d7fb67628355d` passed the complete triggered matrix: Quality Gate #1888, Security Scan #1894, Video Rounded Rectangle Parity Evidence #3, retained playback/cursor/resource-text/pixelate evidence, and every applicable Linux/macOS/browser/container assurance workflow.

Current implementation slice: **canonical rounded-rectangle normal playback** in PR **#314** on branch `feat/video-wysiwyg-phase3-rounded-rectangle-playback`, created directly from #311's actual squash result. The browser admission predicate mirrors the export-proven static-2D `rounded_rectangle` subset and reuses `CanonicalPreviewShape` as the visible canonical consumer. Unsupported shape kinds or parent/crop/effect/transition/camera/animation debt revoke authority for the complete visual frame to `legacy-time-fallback`; no preview-only shape approximation is promoted.

`parity-playback-canonical-v7` is additive to immutable v6 and contains 21 retained cases: every v6 media/text/weighted/cursor case, one canonical rounded-rectangle playback case, and one unsupported ellipse control. The final fixture keeps the rounded-rectangle success sample at frame `6` / 200 ms for a 350 ms observation, leaving 650 ms before its 1.2 s clip end. The unsupported ellipse occupies 9.7–10.6 s and is observed at frame `300` / 10.0–10.3 s, inside both its authored window and the physically generated 24-second media duration.

Implementation head `c9f8972201e49fbeb87c0a32fc7186026b305982` passed the complete triggered matrix before this tracker-only update: Quality Gate #1912, Security Scan #1919, Video Playback Canonical Parity Evidence #128, Video Playback Shape Canonical Parity Evidence #19, Video Rounded Rectangle Parity Evidence #19, Video Cursor Parity Evidence #34, Video Cursor Smoothing Parity Evidence #21, Video Resource Text Parity Evidence #40, pixelate evidence #149/#96/#114/#66, and every applicable Linux/macOS/browser/container assurance workflow. Quality #1912 included backend formatting/vet/tests/race detector, frontend lint/unit/performance/build, the full Playwright smoke suite, Windows/macOS confinement and desktop checks, Helm validation, and the immutable video renderer parity baseline.

The retained v6 and v7 captures both passed on their first attempt on `c9f89722...`; no scheduler retry artifact was needed. v6 artifact `10038006209` has digest `sha256:6e4f38aca83bd900dc0e9eb2e0f77ce29fcc014d5aadcdc05927d069368bab7d`. v7/shape artifact `10038004170` has digest `sha256:075528374e6785229dd76dc2946adf9874020294831a6f3b6f5aa7e92301a325`. Dedicated v7 observations retain rounded rectangle as `canonical-inline` / `canonical-frame` through approximately 0.32–0.68 s and the ellipse as visible `legacy-time-fallback` through approximately 10.04–10.36 s with exact reason `shape-playback-deferred:playback-shape-ellipse:shape-kind-unsupported`.

The v6 and v7 generic evidence workflows now allow at most three unchanged-gate attempts only when a strict verifier proves observation-count underflow or authored-boundary scheduler drift with at least four pre-boundary observations satisfying every expected weighted/text/cursor semantic. Any other failure exits immediately. The dedicated v7 shape-consumer gate is stricter still: only pure observation-count underflow is retryable; any shape consumer, geometry, state, or mode mismatch fails immediately. This tracker commit changes the PR head, so #314 remains merge-gated until the exact tracker-bearing SHA repeats the complete triggered matrix.
...
- `FidelityRenderer` no longer turns the supported cursor path into a sampled Unicode pointer plus rectangle click approximation. The admitted subset calls the backend `cursor-state-v1` or `cursor-state-v2` evaluator for each exact output frame and emits deterministic render-only raster assets.
...
### Canonical rounded-rectangle export parity slice
...
- Exact final #311 head `a4ff85ef0ab5caa47d0f21dda22d7fb67628355d` passed the complete triggered matrix, including Video Rounded Rectangle Parity Evidence #3, before squash merge `f365da67448a0c3b58eba2727dc6faf3da89ca40`.
- Strict parity permits only this same exact subset; all other shape geometry continues to report annotation parity debt.

### Canonical rounded-rectangle normal-playback parity slice

- #314 adds `previewShapePlayback.ts` as a fail-closed browser admission predicate aligned to the exact export-proven static-2D rounded-rectangle subset. It does not broaden authored `shape-state-v1`.
- Whole-frame playback canonicalization admits a supported shape only when its canonical shape state is available and every other active visual consumer is independently admissible for the same output frame. Unsupported ellipse/shape debt returns the entire frame to `legacy-time-fallback`.
- `ShapePreview.tsx` exposes evidence-only playback consumer/state/geometry markers and reuses `CanonicalPreviewShape`; compatibility playback remains on the established legacy time-domain shape painter.
- Focused Vitest coverage locks export-aligned structural admission plus atomic mixed-frame fallback. The v7 backend fixture preserves every v6 case, keeps the rounded success sample early enough to retain a 650 ms end guard, and places the ellipse control at 9.7–10.6 s with its 10.0–10.3 s observation fully inside the generated-media lifetime.
- Generic v6/v7 evidence retries are fail-closed and scheduler-only; the unchanged semantic gate must still pass. The dedicated shape-consumer gate permits retries only for pure observation-count underflow, never for shape consumer/geometry/state errors.
- Exact implementation head `c9f8972201e49fbeb87c0a32fc7186026b305982` passed v6 playback #128 and v7 shape playback #19 on their first capture attempts. v6 artifact `10038006209` is `sha256:6e4f38aca83bd900dc0e9eb2e0f77ce29fcc014d5aadcdc05927d069368bab7d`; v7 artifact `10038004170` is `sha256:075528374e6785229dd76dc2946adf9874020294831a6f3b6f5aa7e92301a325`.
- The same implementation head passed Quality #1912, Security #1919, rounded-rectangle export #19, retained cursor/smoothing/resource-text/pixelate gates, the complete Playwright smoke suite, renderer baseline, and all applicable platform/sandbox assurance. This tracker-only commit must repeat the triggered matrix before #314 merges.
...
|---|---|---|
| Phase 0 — Reproducible parity baseline | **In progress** | Deterministic 103-frame visual/audio/delivery evidence exists. Focused retained controls include opaque PNG exactness, H.264 decoded-frame/±3 color, partial-alpha PNG ±1, transparent VP9 presentation/±4, project-background ±1, normal-playback media/weighted/text/cursor/rounded-rectangle ownership, immutable FFmpeg project-font selection, resource-backed Chromium↔FFmpeg glyph acceptance, cursor export/smoothing evidence, and rounded-rectangle export evidence. #314 adds retained v7 rounded-rectangle playback evidence while preserving immutable v6. The 103-frame torture fixture still lacks a project-font resource and second-platform retained font/glyph evidence. |
...
| Phase 3 — Shared preview composition | **In progress** | Canonical deterministic media, transforms/view/perspective, geometry, effects, transitions, resource text, pixelate raster classes, free-running media/weighted/text/cursor playback, and supported mixed composition are retained. #311 proves the first exact rounded-rectangle export subset; #314 proves the corresponding fail-closed normal-playback admission on its implementation head and awaits tracker-head revalidation. Remaining shape kinds, further independently evidenced raster classes, and AudioGraph consumption remain before Phase 3 can close. |
...
- `shape-state-v1` owns shape geometry/style. #285–#299 establish the retained pixelate raster classes; #311 adds the first export-proven static-2D rounded-rectangle raster subset and #314 proves its corresponding fail-closed normal-playback admission on the validated implementation head. Other shape kinds remain compatibility debt until independently export-proven.
- `cursor-state-v1` owns exact rational cursor sampling, visibility, scale, highlight/click-ring state, and click proximity. #308 proves bounded static-2D cursor export; #309 admits that export-proven subset during normal playback; #310 adds deterministic `cursor-state-v2` smoothing parity.
...
| #308 | Bounded static-2D cursor export raster parity | `5d465b107f0a91846a9c5ebe853657977767dfb3` |
| #309 | Export-aligned cursor authority during normal playback | `505dda173065a52d218bdb05ead9ebe377699048` |
| #310 | Deterministic cursor smoothing contract and playback/export parity | `14ced79493380a24d6597c8c0b00979d51165710` |
| #311 | Export-proven static-2D rounded-rectangle raster parity | `f365da67448a0c3b58eba2727dc6faf3da89ca40` |
...
`parity-cursor-v1` is the focused static-2D cursor browser↔export acceptance gate. #308 independently proves exact cursor output-frame sampling and raster ownership; #309 promotes the same bounded subset into whole-frame canonical normal playback; #310 adds deterministic smoothing through additive `cursor-state-v2`. Animated/3D/camera/effect/transition parents, click audio, and unbounded expansion remain outside the proven subset.

`parity-rounded-rectangle-v1` is #311's focused Chromium↔FFmpeg acceptance gate for the first static-2D shape geometry subset. It freezes exact canonical shape geometry and a fixture-specific raster envelope without weakening repository-global thresholds.

`parity-playback-canonical-v6` is the retained cursor-smoothing normal-playback fixture. `parity-playback-canonical-v7` is #314's additive shape control: it preserves every v6 case, adds canonical rounded-rectangle ownership, and keeps unsupported ellipse as a whole-frame fallback control. The rounded success observation is frame 6 / 200–550 ms, preserving a 650 ms authored end guard. The ellipse control is 9.7–10.6 s with its observation at 10.0–10.3 s, safely within generated media. Generic scheduler-only retries are fail-closed and the dedicated shape gate retries only pure observation-count underflow.
...
| A renderer-specific painter approximation is mistaken for an exact raster source | #299 explicitly rejects broad shape admission after inspecting current FFmpeg shape approximations. #311 admits only a separately rasterized, retained-evidence rounded-rectangle subset; every additional shape kind still requires export-first geometry/pixel proof. |
...
| Partial cursor support is mistaken for universal cursor parity | Capability remains `Partial`; animated/3D/camera/effect/transition parents, ambiguous overlap, long unbounded expansion, and click audio are not claimed by the canonical export/playback path. |
| Normal playback mixes canonical and legacy semantics inside one visual frame | #300/#301 establish whole-frame media admission/fallback; #303 adds weighted Canvas authority; #304 adds resource-backed text; #305 proves supported mixed composition; #309 adds export-proven cursor; #314 adds only the export-proven rounded rectangle. Any unsupported shape/parent/runtime debt revokes the complete frame instead of leaving canonical surfaces over legacy-time content. |
| CI scheduling hides code state | Only actually executed checks count. |

## Next recommended slice

1. **Revalidate the exact tracker-bearing #314 head, then squash-merge only if immutable `parity-playback-canonical-v6`, additive `parity-playback-canonical-v7`/shape-consumer evidence, Quality/Security, retained renderer controls, and every triggered platform assurance all pass on that same SHA.** The implementation head `c9f89722...` already passed the complete matrix and both retained playback suites on their first capture attempts; the documentation-only head must not inherit that status implicitly.
2. **Create the next branch directly from #314's actual squash result and make ellipse the next export-first annotation geometry slice.** Define the narrowest static-2D ellipse subset that can be rendered from canonical `shape-state-v1` without introducing a second transform/stacking model; unsupported parent/crop/effect/transition/camera/animation semantics must stay compatibility-only.
3. Add focused Chromium↔FFmpeg ellipse evidence and freeze a fixture-specific acceptance envelope before changing renderer capability or normal-playback admission. Only after export parity is retained should ellipse be considered for an additive playback v8 success case; keep an unsupported-shape negative control.
4. In parallel, add a second supported OS/toolchain run for the resource-font fixture when practical. Cursor click audio should not be invented until an authored sound contract exists.
