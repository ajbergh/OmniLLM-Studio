...
**Current status:** Phase 1 is complete. Phase 0 now has automated project seeding, a complete 103-frame known-mismatch baseline, independent browser-preview and snapshot PCM with gated EBU R128, decoded-delivery frame/PTS validation, and a CI artifact job. Audio and delivery gates pass on the current live fixture; hosted CI verification, unsupported-audio-boundary coverage, visual review, and final threshold approval remain.

| Phase | Status | Progress note |
|---|---|---|
| Phase 0 — Parity baseline | In progress | Fixture generation, real-project seeding, 103 named visual captures, passing independent PCM timing/correlation/EBU R128 gates, decoded-delivery frame/PTS checks, mismatch reports, and CI artifact wiring are implemented; first hosted CI evidence, unsupported-audio-boundary coverage, visual review, and threshold approval remain |
...
| CI artifact job | Implemented; hosted run pending | `video-parity-baseline` generates, seeds, captures preview/snapshot frames and PCM, performs a separate delivery render, reports in known-mismatch mode, records toolchain/font metadata, and uploads the complete evidence bundle for 14 days |
...
1. Run and inspect the hosted `video-parity-baseline` job, including its delivery render and schema-v2 EBU R128 gates; confirm pinned Chromium/FFmpeg/font metadata and fix any Linux-only determinism, timeout, or packaging failures.
