...
1. **Merge #305 only after this tracker-bearing exact head re-passes every workflow triggered by the final PR diff, including Video Playback Canonical Parity Evidence, Quality/Security, the Playwright/renderer gates included by Quality, and applicable platform/sandbox assurances.** Implementation-head run #85 is retained evidence, not permission to skip final exact-head validation after this documentation update.
2. **Start the next slice directly from #305's actual squash result and close the resource-backed text browser↔FFmpeg glyph-pixel evidence gap before broadening another painter.** Build a focused deterministic fixture that binds immutable project font bytes to one simple text clip and captures the same canonical frame from Chromium and the FFmpeg diagnostic renderer.
3. Keep the first glyph-pixel gate deliberately narrow: one static face/resource, explicit font size/color/position, no family-name fallback, no animation, no shadow/stroke/background, and no line-height approximation. Require exact font-resource identity and exact frame identity before evaluating pixel metrics. Measure first, then freeze a fixture-specific acceptance envelope rather than weakening repository-global thresholds.
4. Preserve normal-playback v4 as an independent structural/runtime gate while adding text pixel evidence. Keep cursor normal-playback admission deferred until export implements matching cursor semantics, and keep general shape playback deferred until current FFmpeg approximations are replaced or independently proven.