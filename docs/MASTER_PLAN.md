...
- **Dependency/risk:** Capability reporting is runtime/platform-specific. Do not advertise a capability before native enforcement and negative evidence exist. Browser egress validation is not a substitute for arbitrary sandbox socket enforcement. Egress requires a separately enforceable runtime boundary, not just a Broker grant. Linux PID/memory capabilities are dynamic and require the relevant delegated cgroup-v2 controllers. For a positive `resources.memory_bytes`, `memory.max` alone is insufficient for the project contract because cgroup swap is separately governed; #174 therefore sets `memory.swap.max=0`. The #175 read boundary closes Linux content-read redirection through parent/final symlink swaps, but it does not make pathname-based write/delete/revert operations, replacement of the registered root itself, search candidate enumeration, or non-Linux workspace operations race-proof.
- **Next action:** Merge #175 only after its documentation-inclusive exact head passes the focused Ubuntu workspace-path assurance and repository-wide gates; then take the mutation/root-identity TOCTOU work as a separate reviewable slice before moving to destination-enforced egress.
