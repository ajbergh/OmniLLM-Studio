...
- **Dependency/risk:** Capability reporting is runtime/platform-specific. Do not advertise a capability before native enforcement and negative evidence exist. Browser egress validation is not a substitute for arbitrary sandbox socket enforcement. Egress requires a separately enforceable runtime boundary, not just a Broker grant. Linux PID/memory capabilities are dynamic and require the relevant delegated cgroup-v2 controllers. For a positive `resources.memory_bytes`, `memory.max` alone is insufficient for the project contract because cgroup swap is separately governed; #174 therefore sets `memory.swap.max=0`. #175 closes tested Linux content-read redirection through parent/final symlink swaps. #176 extends the same identity discipline through mutation capture, commit, verification, and rollback even when a parent pathname is renamed and replaced with an outside symlink. #177 closes the fresh-operation registered-root replacement gap on Linux by binding the persisted grant to device+inode. Legacy rows are deliberately not auto-backfilled because their original filesystem identity cannot be proven. `WalkDir` enumeration and non-Linux governed workspace identity/path semantics remain open.
- **Next action:** Merge #177 only after its documentation-inclusive exact head passes the focused Ubuntu workspace assurance and repository-wide gates; then take search-enumeration and non-Linux workspace path-race/root-identity work as separate reviewable slices before moving to destination-enforced egress.
