...
>
> Audited against repository `main` through PR #173 (`981691a0`) plus the current PR #174 implementation branch on 2026-08-16. Completed initiatives and superseded plans are in [archive/README.md](archive/README.md). This document deliberately excludes completed work except where it is needed to explain a dependency. macOS sandbox Phase 13 completed in PR #166 (`d52ab16`). Native Chromium browser-egress assurance completed in PR #168 (`76f4f4c`). SQLite foreign-key admission is also complete (schema V51).

## Execution order

1. Continue sandbox correctness/security hardening: complete Linux aggregate memory enforcement in #174, then broader TOCTOU assurance, destination-enforced sandbox egress, and credential consumers. Resolve CPU semantics before enabling a CPU capability; physical-disk accounting remains separate work.
...
- **Dependency/risk:** Capability reporting is runtime/platform-specific. Do not advertise a capability before native enforcement and negative evidence exist. Browser egress validation is not a substitute for arbitrary sandbox socket enforcement. Egress requires a separately enforceable runtime boundary, not just a Broker grant. Linux PID/memory capabilities are dynamic and require the relevant delegated cgroup-v2 controllers. The service manager must place `sandboxd` inside a child cgroup beneath the delegated root so atomic `CLONE_INTO_CGROUP` placement into sibling execution cgroups is authorized. The cgroup-v2 `pids` controller counts tasks, including threads. For a positive `resources.memory_bytes`, `memory.max` alone is insufficient for the project contract because cgroup swap is separately governed; #174 therefore sets `memory.swap.max=0` to prevent anonymous pages from escaping the requested byte ceiling.
- **Next action:** Merge #174 only after its documentation-inclusive exact head passes the dedicated Ubuntu cgroup assurance and repository-wide gates; then move to the next independent hardening slice rather than enabling CPU/disk/egress capabilities without their own proven primitives.
