...
- **Linux CPU enforcement — IMPLEMENTED BEHIND A FALSE CAPABILITY / NATIVE PROMOTION PENDING:** the delegated cgroup manager discovers/enables the `cpu` controller when available without regressing already-working PID/memory isolation, parses aggregate `cpu.stat` `usage_usec`, exposes execution-cgroup kill, and configures a one-core `cpu.max` ceiling only to bound sampling overshoot. `LocalRuntime.Exec` now creates the same atomic execution cgroup for CPU-only requests, establishes a pre-execution aggregate baseline, monitors cumulative CPU in 10 ms intervals, kills the complete cgroup when the budget is exhausted, performs final accounting so short bursts cannot bypass the limit between samples, and returns stable metadata (`cpu_limit_enforced`, `cpu_limit_exceeded`, `cpu_time_limit_ms`, `cpu_usage_us`, and `termination_reason=cpu_quota_exceeded`) distinct from caller cancellation/wall timeout. Native assurance now includes a cgroup-level descendant fan-out test plus an actual Bubblewrap `LocalRuntime.Exec` test using a tiny immutable BusyBox rootfs. **`CPULimit` intentionally remains false until those native tests execute successfully on the exact review head and repository gates pass.**
- **Windows CPU primitives — IN PROGRESS:** a Job Object accounting helper queries `JobObjectBasicAccountingInformation` and derives aggregate `TotalUserTime + TotalKernelTime` CPU usage in application milliseconds. The execution monitor/termination path and descendant-pressure native assurance remain outstanding, so `CPULimit` remains false.
...
Linux's hidden enforcement path is intentionally reachable only through direct runtime construction while `CPULimit=false`; Broker admission continues to reject a non-zero application CPU request because capability reporting has not yet been promoted. This provides an end-to-end native proof point without exposing an unvalidated control to callers. Capability bits may change only after the exact implementation and negative cases below are proven.
