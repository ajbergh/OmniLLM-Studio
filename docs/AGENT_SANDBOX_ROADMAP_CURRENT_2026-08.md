...
| 5 | Workspace registry + grants + durable journal | **IN PROGRESS** | Owner-scoped grants and state-bound journaled mutations exist. #175 binds Linux content opens to pinned directory FDs; #176 binds Linux write/delete/revert transactions to pinned directory identities; #177 binds persisted Linux grants to registered root device+inode. #182 replaces Linux search `WalkDir` enumeration with descriptor-relative traversal and same-lineage candidate reads. Non-Linux root identity/descriptor-relative operations remain open. |
| 6 | Governed workspace tools | **IN PROGRESS** | Read/write/patch/delete/revert/search tools exist. After #175/#176/#177 and the current #182 implementation, the tested Linux governed-workspace path-race surface is descriptor/identity bound. Completion still tracks remaining cross-platform Phase 5 assurance. |
...
| 17 | Adversarial assurance suite | **IN PROGRESS** | Browser-native egress assurance is covered by #168, Windows quota controls have native negative evidence, Linux cgroup-v2 quota assurance covers PID/memory, and the Ubuntu workspace lane exercises #175 reads, #176 mutations, #177 root identity, and #182 search enumeration/symlink/rename negatives ten times per run. |
...
The remaining governed-workspace filesystem hardening is cross-platform: Windows and Darwin root identity/path-race semantics must be designed and proven with platform-native primitives rather than treating Linux device+inode/openat semantics as portable. Linux root identity/search binding remains an internal governed-workspace invariant rather than a general sandbox capability bit.
...
1. Complete and merge Linux search-enumeration hardening (#182) only after its documentation-inclusive exact head passes the combined Ubuntu workspace assurance and repository-wide gates.
2. Design and prove non-Linux workspace root identity/path-race hardening as platform-specific reviewable slices rather than claiming Linux semantics cross-platform.
