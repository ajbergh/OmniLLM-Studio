...
| 5 | Workspace registry + grants + durable journal | **IN PROGRESS** | Owner-scoped grants and state-bound journaled mutations exist. #175 binds Linux read/search content opens to pinned directory FDs; #176 binds Linux write/delete/revert capture/mutation/verification/rollback to one pinned parent-directory identity. #177 persists and verifies Linux registered-root device+inode identity and rejects legacy pathname-only grants until trusted re-registration. Search enumeration races and non-Linux identity/descriptor-relative operations remain open. |
| 6 | Governed workspace tools | **IN PROGRESS** | Read/write/patch/delete/revert tools exist. Linux read/search content is protected by #175; Linux write/delete/revert transactions by #176; fresh-operation Linux root replacement is rejected by the current #177 implementation. Completion still tracks search enumeration and remaining cross-platform Phase 5 assurance. |
...
| 17 | Adversarial assurance suite | **IN PROGRESS** | Continuous negative/platform-native testing across all phases; browser-native egress assurance is covered by #168, Windows quota controls have native negative evidence, Linux cgroup-v2 quota assurance covers PID/memory, and the Ubuntu workspace lane now exercises #175 reads, #176 mutations, and #177 root-identity replacement/legacy-grant negatives ten times per run. |
...
#176 extends descriptor-relative identity through governed mutations. `WriteFile`, delete, and revert acquire a platform mutation target; on Linux this target pins the workspace root and final parent directory, captures the final file with `openat(O_NOFOLLOW)` plus `fstat`, creates replacement files inside the pinned parent, commits them with `renameat`, deletes with `unlinkat`, and uses the same pinned parent for after-state capture and journal-failure rollback.

#177 closes the separate fresh-operation registered-root replacement gap on Linux. `sandbox_workspaces` gains nullable `root_identity`; new Linux registrations persist `device+inode`; `Get` re-canonicalizes the registered root and verifies the current identity before returning the grant; `List` omits stale or legacy Linux grants; and `InternalMount` inherits the same verification through `Get`. Existing rows are not auto-backfilled because the application cannot prove the filesystem identity that was originally granted. They fail closed until trusted code re-registers them. Native negative coverage renames the original root, creates a different directory at the same pathname, and proves `Get`, `List`, and `Resolve` refuse the replacement; a separate legacy-row negative proves re-registration is required. The combined Ubuntu workspace lane runs these cases with the existing read/mutation negatives ten times.

This does **not** close all workspace TOCTOU work. `WalkDir` candidate enumeration remains pathname-based, and non-Linux governed workspace operations/root grants still use their prior pathname identity semantics. Linux device+inode binding is not advertised as a general runtime capability bit; it is a governed-workspace registry invariant on Linux only.
...
1. Complete and merge Linux registered-root identity hardening (#177) only after its documentation-inclusive exact head passes the combined Ubuntu workspace assurance and repository-wide gates.
2. Address remaining pathname-based search enumeration without weakening the descriptor-relative final-content boundary.
3. Design and prove non-Linux workspace root identity/path-race hardening as platform-specific reviewable slices rather than claiming Linux semantics cross-platform.
