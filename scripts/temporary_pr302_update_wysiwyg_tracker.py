...
text, count = re.subn(r'\*\*Last updated:\*\* [^\n]+', '**Last updated:** 2026-09-02', text, count=1)
if count != 1:
    raise SystemExit('Last updated marker not found exactly once')

handoff = '''Latest merged WYSIWYG program PR: **#301 — Retain normal playback canonicalization browser evidence** — squash merge `1f891dc4338853357dcfdead8c073c9c81516bba`, directly from #300's actual squash result `d4049105d49ce4bc8337a3c5236593f503869d5d`.
...
'''
text, count = re.subn(
    r'Latest merged WYSIWYG program PR:.*?\n\nCurrent implementation PR:.*?\n\n(?=\*\*Phase 2)',
    handoff,
    text,
    count=1,
    flags=re.S,
...
if count != 1:
    raise SystemExit('current handoff block not found exactly once')

section = '''### #302 ready weighted normal-playback Canvas promotion
...
if '### #302 ready weighted normal-playback Canvas promotion' not in text:
    if marker not in text:
        raise SystemExit('Phase 3 merged consumer sequence marker missing')
    text = text.replace(marker, '\n' + section + '## Phase 3 merged consumer sequence\n', 1)

row300 = '| #300 | Media-only normal-playback canonical output-frame admission with whole-frame fallback | `d4049105d49ce4bc8337a3c5236593f503869d5d` |\n'
row301 = '| #301 | Retained normal-playback canonicalization browser evidence | `1f891dc4338853357dcfdead8c073c9c81516bba` |\n'
if row301 not in text:
    if row300 not in text:
        raise SystemExit('#300 merged sequence row missing')
    text = text.replace(row300, row300 + row301, 1)

old_lineage = '**#301 is directly from #300 squash `d4049105d49ce4bc8337a3c5236593f503869d5d`**.'
new_lineage = '#301 was directly from #300 and squash-merged as `1f891dc4338853357dcfdead8c073c9c81516bba`; **#302 is directly from #301 squash `1f891dc4338853357dcfdead8c073c9c81516bba`**.'
if old_lineage in text:
    text = text.replace(old_lineage, new_lineage, 1)
elif new_lineage not in text:
    raise SystemExit('lineage marker missing')

v1 = "`parity-playback-canonical-v1` is #301's retained free-running browser control for #300. It proves admitted image/video windows consume advancing canonical output-frame identity while the store/UI and audio clocks remain continuous, and separately proves text, cursor, weighted, mixed, and canonically deferred transition windows fail the whole visual frame back to legacy time with exact reasons and no canonical visual-frame publication.\n"
...
if v2.strip() not in text:
    if v1 not in text:
        raise SystemExit('playback v1 baseline paragraph missing')
    text = text.replace(v1, v1 + v2, 1)
...
'''
text, count = re.subn(r'\n## Next recommended slice\n.*\Z', next_section, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit('Next recommended slice section missing')

required = [
    '#302 ready weighted normal-playback Canvas promotion',
    '9851179481',
    '8447638c4b3aa831ce527d242e79838cf67ba063af9f77be4ba3a1c342e8c330',
    '| #301 | Retained normal-playback canonicalization browser evidence |',
    'parity-playback-canonical-v2',
    "Start the next Phase 3 slice directly from #302's actual squash result",
]
for needle in required:
    if needle not in text:
        raise SystemExit(f'missing required tracker evidence: {needle}')
