...
if (!args.url || !args.fixture || !args['image-file']) {
  console.error('usage: node scripts/video-cursor-smoothing-parity-capture.mjs --url <app-url> --fixture <json> --image-file <png> [--output <dir>]');
  process.exit(1);
}

const fixture = JSON.parse(await fs.readFile(path.resolve(args.fixture), 'utf8'));
if (fixture.name !== 'parity-cursor-smoothing-v2') throw new Error(`unexpected fixture ${fixture.name}`);
const expectedFrames = [25, 50, 75];
if (fixture.timeline?.canvas?.width !== 640 || fixture.timeline?.canvas?.height !== 360 || fixture.timeline?.canvas?.fps !== 100) {
  throw new Error(`cursor smoothing fixture canvas drifted: ${JSON.stringify(fixture.timeline?.canvas)}`);
}
if (fixture.samples?.length !== expectedFrames.length || !fixture.samples.every((sample, index) => sample.frame_index === expectedFrames[index])) {
  throw new Error(`cursor smoothing fixture sample set drifted: ${JSON.stringify(fixture.samples)}`);
}
const authoredCursor = fixture.timeline?.tracks?.[0]?.clips?.[0]?.cursor;
if (!authoredCursor?.smoothing || authoredCursor.events?.length !== 2 || authoredCursor.click_rings) {
  throw new Error(`cursor smoothing fixture authoring drifted: ${JSON.stringify(authoredCursor)}`);
}

const output = path.resolve(args.output || 'output/video-cursor-smoothing-parity/capture');
...
function expectedCursorAt(timeMS) {
  const [first, last] = authoredCursor.events;
  if (timeMS <= first.time_ms) return { x: first.x, y: first.y, progress: 0, smooth_progress: 0 };
  if (timeMS >= last.time_ms) return { x: last.x, y: last.y, progress: 1, smooth_progress: 1 };
  const progress = (timeMS - first.time_ms) / Math.max(1, last.time_ms - first.time_ms);
  const smoothProgress = smoothstep(progress);
  return {
    x: first.x + (last.x - first.x) * smoothProgress,
    y: first.y + (last.y - first.y) * smoothProgress,
    progress,
    smooth_progress: smoothProgress,
  };
}
...
for (const sample of fixture.samples) {
  ...
  const pinned = expectedCenters[sample.frame_index];
  if (!pinned || Math.abs(expected.x - pinned.x) > 1e-12 || Math.abs(expected.y - pinned.y) > 1e-12) {
    throw new Error(`independent smoothstep expectation drifted at frame ${sample.frame_index}: expected=${JSON.stringify(expected)} pinned=${JSON.stringify(pinned)}`);
  }
}
...
try {
  ...
  const project = await jsonRequest('POST', '/video/projects', {
    title: 'Canonical Cursor Smoothing Parity',
  ...
  const upload = await context.request.post(apiURL(`/video/projects/${encodeURIComponent(project.id)}/assets/upload`), {
    headers,
    multipart: { file: { name: 'cursor-smoothing-backdrop.png', mimeType: 'image/png', buffer: imageBytes } },
  });
  ...
  if (downloadedImageSHA256 !== sourceImageSHA256 || downloadedImageBytes.length !== imageBytes.length) {
    throw new Error(`uploaded smoothing backdrop bytes changed: source=${sourceImageSHA256}/${imageBytes.length} downloaded=${downloadedImageSHA256}/${downloadedImageBytes.length}`);
  }
  ...
  const frameEvidence = [];
  for (const sample of fixture.samples) {
    const requestId = `smoothing-${String(sample.frame_index).padStart(6, '0')}-${sample.name}`;
    ...
    const expected = expectedCursorAt(sample.time_ms);
    const linearProgress = sample.time_ms / 1000;
    const linear = { x: 160 + 320 * linearProgress, y: 100 + 160 * linearProgress };
    ...
    const ring = cursorEvidence.child_divs.find((item) => item.border_top_width === '2px');
    if (cursorEvidence.state_mode !== 'canonical-frame') throw new Error(`frame ${sample.frame_index}: smoothing cursor did not use canonical-frame state: ${JSON.stringify(cursorEvidence)}`);
    if (Math.abs(cursorEvidence.left - expected.x) > 0.02 || Math.abs(cursorEvidence.top - expected.y) > 0.02) {
      throw new Error(`frame ${sample.frame_index}: smoothstep drifted expected=${JSON.stringify(expected)} actual=${JSON.stringify(cursorEvidence)}`);
    }
    if ((sample.frame_index === 25 || sample.frame_index === 75)
      && Math.abs(cursorEvidence.left - linear.x) < 1
      && Math.abs(cursorEvidence.top - linear.y) < 1) {
      throw new Error(`frame ${sample.frame_index}: cursor remained linear instead of smoothing: linear=${JSON.stringify(linear)} actual=${JSON.stringify(cursorEvidence)}`);
    }
    ...
    if (!highlight || ring) throw new Error(`frame ${sample.frame_index}: smoothing fixture highlight/ring structure drifted: ${JSON.stringify(cursorEvidence)}`);
  ...
  await fs.writeFile(path.join(output, 'cursor-smoothing-frame-evidence.json'), `${JSON.stringify({ fixture: fixture.name, frames: frameEvidence }, null, 2)}\n`);
