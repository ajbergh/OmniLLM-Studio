...
func timelineAudioProcessingFilters(metadata map[string]any) []string {
	...
	if enabledBool(raw, "denoise") {
		filters = append(filters, "highpass=f=70", "afftdn=nr=12:nf=-35")
	}
	switch strings.ToLower(stringValue(raw, "eq_preset")) {
	case "voice":
	...
	if enabledBool(raw, "compressor") {
		filters = append(filters, "acompressor=threshold=-18dB:ratio=3:attack=15:release=180:makeup=2")
	}
	if enabledBool(raw, "normalize") {
		target := numberValue(raw, "target_lufs", -16)
		if target > -5 {
			target = -5
		}
		if target < -30 {
	...
	if enabledBool(raw, "limiter") {
		filters = append(filters, "alimiter=limit=0.95:attack=5:release=50")
	}
	switch strings.ToLower(stringValue(raw, "channels")) {
	case "mono":
