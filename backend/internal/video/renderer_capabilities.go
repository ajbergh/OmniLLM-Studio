...
func FFmpegRendererCapabilities() RendererCapabilities {
	return RendererCapabilities{
		...
		Features: []RendererFeatureSupport{
			...
			{Feature: RendererFeatureCursor, Label: "Cursor effects", Supported: true, Partial: true, Notes: "Static 2D media cursors up to the bounded fidelity segment limit use cursor-state-v1 output-frame sampling plus deterministic pointer, highlight, and click-ring rasters on the owner track. Smoothing, animated/3D/camera/effect/transition parents and longer clips retain the sampled compatibility fallback. Click audio is not synthesized."},
