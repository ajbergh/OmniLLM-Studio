...
// contract. It deliberately excludes media, audio, decoration, animation,
// explicit line-height, fallback families, and non-black canvas color so
// retained differences measure Chromium/FFmpeg glyph rasterization against one
// immutable font face rather than unrelated color-conversion or timing noise.
func ParityResourceTextFixture() (TimelineDocument, []ParityFixtureAsset) {
	...
	return doc, []ParityFixtureAsset{{
		ID:          "asset-font",
		Kind:        "font",
		Description: "immutable regular DejaVu Sans face bound to parity-text-face-v1",
	}}
}

// ParityResourceTextFrameSamples binds measurement to one stable interior
// output frame. The fixture is static, so one frame removes temporal noise while
// still proving the canonical frame-addressed preview/export path.
