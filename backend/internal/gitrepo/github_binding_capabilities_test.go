...
func TestRemoteServiceCapturesGitHubBindingCapabilitiesAtConstruction(t *testing.T) {
	t.Setenv(RepositoriesEnv, "omni="+t.TempDir())
	t.Setenv(RemotesEnv, "")
	t.Setenv(RemoteEnabledEnv, "true")
	t.Setenv(GitHubBindingCapabilitiesEnv, `{"omni":{"allow_pull_request_read":true}}`)

	local := NewServiceFromEnvironment()
	...
	if !ok || !policy.AllowPullRequestRead {
		t.Fatalf("binding capability policy was not captured: %#v", service.githubBindingCapabilities)
	}

	t.Setenv(GitHubBindingCapabilitiesEnv, `{}`)
	policy, ok = service.githubBindingCapabilities["omni"]
	if !ok || !policy.AllowPullRequestRead {
		t.Fatal("constructed remote service must keep its startup policy snapshot")
	}
}
