...
func runAgentWorktreeTestGit(t *testing.T, root string, args ...string) {
	t.Helper()
	cmd := exec.Command("git", append([]string{"-C", root}, args...)...)
	cmd.Env = append(os.Environ(), "GIT_CONFIG_NOSYSTEM=1", "GIT_TERMINAL_PROMPT=0")
	if output, err := cmd.CombinedOutput(); err != nil {
...
func TestAgentWorktreePromotionRejectsAdvancedTargetHEAD(t *testing.T) {
	repoRoot := initAgentWorktreeTestRepo(t)
	service := NewServiceWithWriteAccess(map[string]string{"omni": repoRoot}, true)
	...
	owner := AgentWorktreeOwner{UserID: "user-1", WorkspaceID: "workspace-1", AgentRunID: "run-advanced-head"}
	worktree, err := manager.Create(context.Background(), "omni", owner, "HEAD")
	if err != nil {
		t.Fatal(err)
	}
	...
	if review.BaseCommit != worktree.BaseCommit || review.PatchSHA256 == "" {
		t.Fatalf("unexpected review binding: %#v", review)
	}

	// Advance the real repository with an unrelated clean commit. The reviewed
	// tracked.txt patch remains applicable, but its immutable base identity is
	// stale and promotion must therefore fail closed.
	if err := os.WriteFile(filepath.Join(repoRoot, "unrelated.txt"), []byte("new base commit\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	runAgentWorktreeTestGit(t, repoRoot, "add", "unrelated.txt")
	runAgentWorktreeTestGit(t, repoRoot, "commit", "-m", "advance target")

	if err := manager.Promote(context.Background(), worktree.ID, owner, review.PatchSHA256); err == nil || !strings.Contains(err.Error(), "HEAD changed") {
		t.Fatalf("expected advanced target HEAD rejection, got %v", err)
	}
	tracked, err := os.ReadFile(filepath.Join(repoRoot, "tracked.txt"))
	if err != nil || string(tracked) != "base\n" {
		t.Fatalf("stale-base promotion mutated tracked file = %q err=%v", tracked, err)
	}
}
