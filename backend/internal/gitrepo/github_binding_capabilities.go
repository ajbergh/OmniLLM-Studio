...
const (
	// GitHubBindingCapabilitiesEnv contains operator-owned configuration for
	// binding-derived GitHub remotes. Keys are startup-allowlisted local
...
// GitHubBindingCapabilities is the explicit operator configuration for one
// binding-derived GitHub remote. The first schema intentionally excludes
// default-branch push and clone.
...
// binding configuration. Invalid entries are ignored independently so one bad
// entry cannot alter another. Unknown fields reject only the affected entry.
