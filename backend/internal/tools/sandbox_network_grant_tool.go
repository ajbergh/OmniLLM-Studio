...
func (t *SandboxNetworkGrantTool) Validate(raw json.RawMessage) error {
	...
	if len(args.Domains) == 0 || len(args.Domains) > 16 { return fmt.Errorf("domains must contain 1-16 entries") }
	if len(args.Ports) > 16 { return fmt.Errorf("ports must contain at most 16 entries") }
	if args.TTLSeconds != 0 && (args.TTLSeconds < 60 || args.TTLSeconds > 1800) { return fmt.Errorf("ttl_seconds must be between 60 and 1800") }
	return nil
}
...
func (t *SandboxNetworkGrantTool) Execute(ctx context.Context, raw json.RawMessage) (*ToolResult, error) {
	...
	ttl := 15 * time.Minute
	if args.TTLSeconds > 0 { ttl = time.Duration(args.TTLSeconds) * time.Second }
	grant, err := sandbox.DefaultNetworkGrantStore().Create(owner, args.Domains, args.Ports, ttl)
	if err != nil { return nil, err }
	structured, _ := json.Marshal(grant)
	return &ToolResult{Content: string(structured), Structured: structured, Metadata: map[string]interface{}{"grant_id": grant.ID, "expires_at": grant.ExpiresAt}}, nil
}
