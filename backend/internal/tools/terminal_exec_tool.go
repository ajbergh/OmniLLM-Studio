...
// source mutations remain journaled workspace-tool operations.
...
func (t *TerminalExecTool) Definition() ToolDefinition {
	return ToolDefinition{
		Name: "terminal_exec",
		Description: "Run a command with explicit argv inside the configured OS sandbox. An optional granted filesystem workspace is mounted read-only; use workspace_write/workspace_apply_patch for journaled source changes. Network is disabled.",
		Category: "compute",
		...
		MaxResultBytes: 1 << 20,
		Parameters: json.RawMessage(`{"type":"object","properties":{"command":{"type":"string","minLength":1},"args":{"type":"array","maxItems":128,"items":{"type":"string","maxLength":8192}},"workspace_id":{"type":"string"},"directory":{"type":"string"},"stdin":{"type":"string","maxLength":1048576},"timeout_ms":{"type":"integer","minimum":100,"maximum":60000}},"required":["command"],"additionalProperties":false}`),
	}
}
...
func (t *TerminalExecTool) Execute(ctx context.Context, raw json.RawMessage) (*ToolResult, error) {
	...
	spec := sandbox.CreateRequest{
		...
		Resources: sandbox.ResourceLimits{WallTimeMS: args.TimeoutMS, MaxStdoutBytes: 1 << 20, MaxStderrBytes: 1 << 20},
		TTLSeconds: 120,
		Requirements: sandbox.RuntimeRequirements{OSIsolation: true, FilesystemIsolation: true, NetworkIsolation: true, ProcessTreeIsolation: true},
	}
	...
	out, execErr := broker.Exec(ctx, owner, session.ID, sandbox.ExecRequest{
		Command: strings.TrimSpace(args.Command), Args: append([]string(nil), args.Args...), Directory: args.Directory,
		Stdin: []byte(args.Stdin), TimeoutMS: args.TimeoutMS,
	})
	...
	metadata := map[string]interface{}{"session_id": session.ID, "execution_id": out.ExecutionID, "exit_code": out.ExitCode, "network": "none", "workspace_write": false}
	for key, value := range out.Metadata { metadata[key] = value }
	return &ToolResult{Content: content, Structured: structured, IsError: out.ExitCode != 0, Metadata: metadata}, nil
}
