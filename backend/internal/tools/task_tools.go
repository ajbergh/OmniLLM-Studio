...
// TaskListTool lists scheduled tasks owned by the current user.
type TaskListTool struct{ scheduler *tasks.Scheduler }

func NewTaskListTool(scheduler *tasks.Scheduler) *TaskListTool { return &TaskListTool{scheduler: scheduler} }
...
func (t *TaskListTool) Definition() ToolDefinition {
	return ToolDefinition{
		...
		Category: "automation", Enabled: t.scheduler != nil, Version: "1", Risk: RiskLow,
...
func (t *TaskListTool) Validate(raw json.RawMessage) error {
...
func (t *TaskListTool) Execute(ctx context.Context, raw json.RawMessage) (*ToolResult, error) {
	var args struct{ Limit int `json:"limit"` }
	if err := json.Unmarshal(raw, &args); err != nil {
		return nil, err
	}
	items, err := t.scheduler.List(InvocationScopeFromContext(ctx).UserID, args.Limit)
	if err != nil {
		return nil, err
	}
	encoded, _ := json.Marshal(items)
	if len(items) == 0 {
		return &ToolResult{Content: "No scheduled tasks.", Structured: encoded}, nil
	}
...
// TaskCreateTool creates an automation and requires approval.
type TaskCreateTool struct{ scheduler *tasks.Scheduler }

func NewTaskCreateTool(scheduler *tasks.Scheduler) *TaskCreateTool { return &TaskCreateTool{scheduler: scheduler} }
...
func (t *TaskCreateTool) Definition() ToolDefinition {
	return ToolDefinition{
		...
		Category: "automation", Enabled: t.scheduler != nil, Version: "1", Risk: RiskHigh,
...
func (t *TaskCreateTool) Validate(raw json.RawMessage) error {
...
func (t *TaskCreateTool) Execute(ctx context.Context, raw json.RawMessage) (*ToolResult, error) {
	var args taskCreateArgs
	if err := json.Unmarshal(raw, &args); err != nil {
		return nil, err
	}
	...
	item, err := t.scheduler.Create(tasks.CreateRequest{
		UserID: scope.UserID, ConversationID: conversationID, Title: args.Title,
		Prompt: args.Prompt, Profile: profile, Timezone: args.Timezone,
		ScheduleKind: args.ScheduleKind, NextRunAt: nextRun, IntervalSeconds: args.IntervalSeconds,
	})
	if err != nil {
		return nil, err
	}
	encoded, _ := json.Marshal(item)
	return &ToolResult{Content: fmt.Sprintf("Created scheduled task %s. Next run: %s.", item.ID, item.NextRunAt.Format(time.RFC3339)), Structured: encoded, Metadata: map[string]interface{}{"task_id": item.ID}}, nil
}

// TaskUpdateTool pauses, resumes, or deletes a task and requires approval.
type TaskUpdateTool struct{ scheduler *tasks.Scheduler }

func NewTaskUpdateTool(scheduler *tasks.Scheduler) *TaskUpdateTool { return &TaskUpdateTool{scheduler: scheduler} }
...
func (t *TaskUpdateTool) Validate(raw json.RawMessage) error {
	var args struct {
...
func (t *TaskUpdateTool) Execute(ctx context.Context, raw json.RawMessage) (*ToolResult, error) {
	var args struct {
		TaskID string `json:"task_id"`
		Action string `json:"action"`
	}
	...
	userID := InvocationScopeFromContext(ctx).UserID
	var err error
	switch args.Action {
	case "pause":
	...
	return &ToolResult{Content: fmt.Sprintf("Task %s: %s complete.", args.TaskID, args.Action), Structured: encoded}, nil
}
