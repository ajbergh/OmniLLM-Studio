...
type darwinWorkspaceMutationTarget struct {
	parentFD int
	name     string
}
...
func openWorkspaceMutationTarget(root, relativePath string) (workspaceMutationTarget, error) {
	...
	if err := requireDarwinDirectoryFD(rootFD); err != nil {
		_ = unix.Close(rootFD)
	...
	for _, part := range parts[:len(parts)-1] {
		...
		if err := requireDarwinDirectoryFD(nextFD); err != nil {
			_ = unix.Close(nextFD)
			_ = unix.Close(rootFD)
	...
	return &darwinWorkspaceMutationTarget{parentFD: parentFD, name: parts[len(parts)-1]}, nil
}

func (t *darwinWorkspaceMutationTarget) Capture() (FileState, error) {
	...
	return FileState{Exists: true, SHA256: hex.EncodeToString(hash[:]), Mode: os.FileMode(stat.Mode & 0o777), Content: data, Revertable: true}, nil
}

func (t *darwinWorkspaceMutationTarget) Write(data []byte, mode os.FileMode) error {
	tempName, err := randomDarwinWorkspaceTempName()
	if err != nil {
		return err
	}
...
func (t *darwinWorkspaceMutationTarget) Delete() error {
...
func (t *darwinWorkspaceMutationTarget) Restore(state FileState) error {
	if !state.Exists {
...
func (t *darwinWorkspaceMutationTarget) Close() error {
	if t.parentFD < 0 {
		return nil
	}
...
func randomDarwinWorkspaceTempName() (string, error) {
	var random [16]byte
