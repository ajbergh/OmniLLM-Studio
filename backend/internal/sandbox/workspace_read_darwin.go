...
// readWorkspaceRegularFile opens each requested component relative to an
// already-open directory descriptor. O_NOFOLLOW keeps the read bound to the
// granted directory lineage even if pathname entries are concurrently swapped.
...
func requireDarwinDirectoryFD(fd int) error {
	var stat unix.Stat_t
