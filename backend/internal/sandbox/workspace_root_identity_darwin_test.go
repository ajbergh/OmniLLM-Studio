...
func TestDarwinWorkspaceGrantRejectsReplacedRootIdentity(t *testing.T) {
	...
	if workspace.RootIdentity == "" || !strings.HasPrefix(workspace.RootIdentity, "darwin:") {
...
func TestDarwinWorkspaceLegacyGrantRequiresReregistration(t *testing.T) {
	registry := newTestWorkspaceRegistry(t)
	root := t.TempDir()
	...
	if !strings.HasPrefix(workspace.RootIdentity, "darwin:") {
